...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.campaign.libraries["feats"] = [STARTER_FEAT]
        self.campaign.libraries["conditions"] = [FATIGUED_CONDITION]
    ...
    def import_feats_txt(self) -> None:
        ...
        improved = next((f for f in self.campaign.libraries["feats"] if f["name"].lower() == "improved initiative"), None)
        if improved:
            improved["mappingStatus"] = "Mapped"
            improved["modifiers"] = [{"target": "initiative", "value": 4, "bonusType": "untyped"}]

        QMessageBox.information(self, "Import complete", f"Imported {len(parsed['rows'])} feats.")
        self.refresh_all()
    ...
    def import_mapping_json(self) -> None:
        ...
        for mapping in payload.get("mappings", []):
            feat = next((f for f in self.campaign.libraries["feats"] if f.get("id") == mapping.get("featId") or (f.get("name") == mapping.get("featName") and f.get("source") == mapping.get("source"))), None)
            if feat:
                feat["mappingStatus"] = "Mapped"
                feat["modifiers"] = mapping.get("modifiers", [])
                feat["prerequisites"] = mapping.get("prerequisites", [])
        self.refresh_all()
    ...
    def load_campaign_dialog(self) -> None:
        ...
        if not self.campaign.libraries.get("feats"):
            self.campaign.libraries["feats"] = [STARTER_FEAT]
        if not self.campaign.libraries.get("conditions"):
            self.campaign.libraries["conditions"] = [FATIGUED_CONDITION]
